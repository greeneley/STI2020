...
int main (void)
{
	...
	sigset_t ensemble;// type "pact" : eo quan tam biet nhung gi ben trong nhung ma ctrinh se dung

	action.sa_handler = gestionnaire;
	sigfillset (& (action . sa_mask)); // il va bloquer tout les signaux sauf que
	action.sa_flags = 0; /* Pas de SA_RESTART */ // mettre tout les flags = 0
	for (i = 1; i < NSIG; i ++)
	...
	sigfillset(& ensemble); // intialisize set to full, all signal
