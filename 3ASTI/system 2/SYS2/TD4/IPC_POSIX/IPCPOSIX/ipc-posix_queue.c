...
#define MSGQOBJ_NAME    "/bal" // boitre aux letter -> dans le systeme de fichier je trouve un fichier bal
...
if (argc==1) usage(argv[0]); 
 switch(atoi(argv[1])) {
                case 1: // create file message mqueue
...
printf("OK\n");
    ...
    printf("File \"%s\":\n\t- capacité maxi %ld messages\n\t- longueur maxi %ld \n\t- message courant %ld \n", MSGQOBJ_NAME, msgq_attr.mq_maxmsg, msgq_attr.mq_msgsize, msgq_attr.mq_curmsgs);
		...
		case 4: // detruit le boitre aux lettre
			msgq_id=open_bal();
    			mq_getattr(msgq_id, &msgq_attr);
