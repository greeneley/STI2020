...
extern double moyenne(double t[],int n);
extern double moyenne_positif(double t[]);
extern double test_moyenne();
extern int chaine_longueur_rec(char *s);
extern int chain_debute_par(char *s1, char *s2);
extern int chain_indice(char *s1,char *s2);
extern void elever_au_cude(int* p);
# 8 "root.c" 2

int proche(double a, double b){
 return fabs(a)<1e-100 ? fabs(b)<1e-100:fabs(a-b)<1e-10*fabs(a);
}
...
int chaine_longueur_rec(char *s){
 if(*s==0)
  return 0;
 else 1+chaine_longueur_rec(s+1);
}
int chain_debute_par(char *s1, char *s2){
 if (*(s2+0)==0)
   return 1;
 else
 if(*s2!=*s1)
    return 0;
 else chain_debute_par(s1+1,s2+1);
 }

int chain_indice(char *s1,char *s2){
 int i;
 for (i=0; s1[i]!='\0';++i)
   if(chain_debute_par(s1+i,s2))
 return i;
 return -1;
}
